	...
	<div class="con">
		<table width="600" border="0" cellspacing="0" cellpadding="0" class="ad_car">
			<tr>
				...
				<td>
					<p>
						...
						<c:if test="${!empty draw.img}">
							<img src="<%=imgPath%>${draw.img}" width="192" height="85" class="img" />
						</c:if>
					</p> 
			...
			<tr>
				...
				<td>
					<textarea id="remark_text" name="remark_text" cols="45" rows="5" class="benzhu">${draw.remark}</textarea>
				</td>
			</tr>
			<tr>
				<td></td>
				<td>
					<form action="<%=basePath%>shop/editDraw" id="add_ajax" method="post">
						<input type="hidden" name="id" id="id" value="${draw.id }" />
						<input type="hidden" name="shopid" id="shopid" value="${draw.shopid }" /> 
						<input type="hidden" name="img" id="img" value="${draw.img }"/> 
